<template>
	<view  class="jsfun-picker" @tap="showPicker">
	 ...
	 <view class="picker" :class="{'pickerShow':isShow}" catchtouchmove="true">
		 <!-- 标题 -->
		 <view class="title x-r X-RautoAdjust">
		 		 <text style="font-size: 35upx;" @tap.stop="closePicker">取消</text>
		 		 <text style="font-size: 35upx; color:#304E07" @tap.stop="okClick">确定</text>
		 </view>
		 <!-- 数据列表 -->
...
<script>
	export default {
		...
		methods: {
			showPicker(){
				if(!this.firstOpen){ this.initValue(); this.firstOpen = true; } setTimeout(() => { this.isShow = true; }, 100);
